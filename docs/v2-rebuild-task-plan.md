...
本文件记录 `dev2` 的受控重建任务。v2 采用全新重写方式，但不是盲目删除重做，而是以 v1 功能、现有问题台账、现场验证经验和待办文档作为需求来源，重新构建更清晰的模块边界、任务状态和升级体系。
...
- 分支边界：只在 `dev2` 上重建，不影响 `dev/main`。
- 测试边界：v2 构建、部署和现场验证可以使用 `10.20.11.3`，远端仓库必须切换到 `dev2` 分支。
...
- [x] 风险提示面板展示风险集群主要增长 VM，并支持点击 VM 跳转虚拟机页面。
...
- [x] Word 目录切换为原生 TOC 字段，主章节为 `Heading 1`，集群章节为 `Heading 2`，集群内 TOP 表标题为 `Heading 3`，并开启打开文档时更新目录。
...
- [x] 保留“完整迁移包”：SQLite 只导出 `towers/clusters` 配置，同时包含 Prometheus 历史指标，用于提升导出速度并保留趋势/增长/预测历史。
...
- Prometheus 历史 block。
- SQLite 迁出瘦身为 `towers/clusters`，不迁出 `vm_latest`、`vm_volumes`、`collection_runs`、`metric_snapshots` 等运行态缓存。
- SQLite 文件。
- 升级包。
- 升级前备份。
- 迁出包。
...
- [x] Dashboard 容量风险 top clusters 附加 `top_growth_vms`，底部风险提示可直接定位主要增长 VM。
- [x] 日增长和本日新建 VM 后端基础。
- [x] VM 列表和趋势后端基础。
- [x] VM 详情和卷信息后端基础。
...
- [x] Word 运维建议使用离线规则模板生成，不依赖 AI 服务。
- [x] Word 表格直接参考客户版模板样式，保留 v2 图表和数据口径，并补充虚拟机数量。
- [x] Word 二次优化：去除官方平台误称，封面增加空白客户名称，统一开源字体，支持多 Tower/多集群范围表达，统计窗口按真实样本窗口收敛，重点文字加粗放大但数据值不额外加空格，2.2 增加 Tower 列，2.3 改为 DOCX 纯文本块容量使用率可视化。
- [x] Word 第三轮口径修正：按采集窗口标注增长口径，范围基本信息支持多 Tower，多处正文 Tower/集群名称加粗放大。
- [x] Word/Excel 六种导出窗口均使用同一 Profile 体系，避免自然月/季度/年度误读。
- [x] Excel 客户版模板化：使用 `存储容量预测分析报表_客户版.xlsx` 作为样式母版，加载模板后填充 v2 数据，保留现有导出 API 和任务中心链接；清理模板中的错误品牌、客户名、样例数据和固定单位。
- [x] Word 章节重排：封面后第二页插入目录；`一  执行摘要` 改为 `一  摘要`；`三`、`四` 按集群分开汇报并生成目录项；`五` 改为所有集群汇总报告。
...
- [x] 服务重启第一版。
- [x] 空间扫描和清理第一版。
- [x] SQLite 清理并整理扫描和 VACUUM 第一版。
- [x] SQLite 备份清理第一版，支持扫描、勾选和删除指定 SQLite 数据库备份。
- [x] 任务中心基础联动。
...
- [x] pre_install。
- [x] 升级包和组件包打包第一版。
- [x] 在 `10.20.11.3` 切换到 `dev2` 后完成 v2 第一版构建和运行验证。
...
- [x] 在 `10.20.11.3` 部署 v2，并确认远端分支为 `dev2`。
- [x] 执行 `pre_install.sh`。
- [x] 启动 compose。
- [x] 添加 Tower 并采集。
- [x] 导出 Word/Excel。
