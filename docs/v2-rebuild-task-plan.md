...
- 新增“配置迁移包”：只导出 `towers` 和 `clusters`，manifest 标记 `migration_scope=config`。
- 保留“完整迁移包”：包含 SQLite 必要数据和 Prometheus 历史指标，manifest 标记 `migration_scope=full`。
- 配置迁移包导入前仍先备份当前 `smartx.db`，导入时只 merge `towers/clusters`。
- 配置迁移不迁移平台管理员账号、任务历史、VM/卷缓存、采集记录或当前指标快照。
...
- 任务中心分为 `info`、`warning`、`critical` 三类通知。
- 角标统计未处理通知，不统计运行中任务数量。
- 信息类完成任务在打开任务中心并点击空白关闭后标记已读。
- 告警和严重告警必须点击“确认”或 X 删除后才清除角标。
- 一键清空只清理已读信息任务和已确认告警任务。
- 失败/取消任务右侧展示“确认”和 X 两个操作，确认按钮位于 X 左侧。
