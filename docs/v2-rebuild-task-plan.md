...
- [ ] 任一集群使用率 `>= 80%` 判定高风险。
- [ ] 任一集群使用率 `75%-80%` 判定需关注。
- [ ] 无风险时显示 `当前所有集群暂无明显容量风险`。
- [ ] 容量风险卡片放在首页最前面。
- [ ] Tower 和集群卡片保持独立，布局不影响其他卡片。
- [ ] 日增长最快 VM 支持增长量/增长率切换。
- [ ] 本日新建 VM 做成独立卡片，放在日增长下方。
- [ ] VM 项点击跳转虚拟机页面并定位。
...
- [ ] 重建 VM 列表接口。
- [ ] 重建 VM 趋势接口。
- [ ] 趋势查询支持 7/14/30/90/180/365 天。
- [ ] 趋势查询按 `tower_id + cluster_id + vm_id` 过滤。
- [ ] 重建 VM 卷详情接口。
- [ ] 前端支持从 Dashboard 和报表跳转到指定 VM。
- [ ] VM 改名后展示最新名称。
...
- [ ] 设计统一任务状态：pending、running、success、failed、cancelled。
- [ ] 设计统一任务步骤：名称、状态、进度、日志、开始时间、结束时间。
- [ ] 任务状态持久化到 `/data` 或 SQLite，确保 web-api 重启后可恢复。
- [ ] 前端右上角任务入口展示后台任务。
- [ ] 点击空白处可收起任务菜单。
- [ ] 任务完成后保留结果和下载链接。
- [ ] 失败任务展示失败步骤和错误摘要。
