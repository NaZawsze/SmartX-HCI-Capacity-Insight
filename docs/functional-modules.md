...
- 需要区分平台升级和组件升级
- [已解决] v2 已基于历史升级问题重新设计平台升级和组件升级模式，形成统一 manifest、状态机、包格式、备份、回滚和组件识别第一版。
...
v2 在 `dev2` 上全新重写，但功能边界仍继承本文件的模块归类。实现时按下面映射拆分，不再把所有逻辑堆到少数大文件里。
...
- Phase V2-0 先冻结文档。
- Phase V2-1 起再进入代码重建。
- 后续 v2 构建、部署和验证可使用 `10.20.11.3`，远端仓库必须切到 `dev2`。
