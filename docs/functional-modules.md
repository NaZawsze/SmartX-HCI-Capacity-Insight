...
- 日增长最快 VM 无数据
- [待处理] 报表页/增长区域需要在“日增长最快 VM”下新增“本日新建 VM”，并支持点击跳转到虚拟机页面
...
- [待处理] 验证 VM 改名后最新展示名称是否跟随 Tower 最新采集结果同步；历史数据必须继续按 `tower_id + cluster_id + vm_id` 绑定
...
- [待处理] 月增长最快 VM 需要排除历史样本不足 30 天的 VM；刚部署没有满足 30 天样本时月增长榜为空
- [待处理] Word/Excel 导出的月增长 TOP VM 也需要使用“样本满 30 天”过滤口径
- [待处理] Word/Excel 导出中的“上期容量”需要在表头或说明里标注统计窗口起止日期，建议改为“期初容量”，并显示类似 `2026年05月01日-2026年05月31日`
- [待处理] 报表页需要在“月增长最快 VM”下新增“本月新建 VM”，并支持点击跳转到虚拟机页面
...
- [待处理] 数据迁移导入前应先生成当前系统备份，备份成功后才继续导入；备份失败默认阻止导入。
