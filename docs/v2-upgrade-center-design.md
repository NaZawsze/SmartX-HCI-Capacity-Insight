...
OVA 虚拟机模板是全新部署或演示环境交付制品，不属于升级中心包。升级中心只处理本节定义的 `.tar.gz` 平台升级包、Runner 组件包、Prometheus 观测包和组合包。OVA 制品边界见 [OVA 交付说明](ova-delivery.md)。
...
平台升级、runner 组件升级和 Prometheus 观测组件升级完成后，不再自动执行平台级自检，也不再写入 `self_check_status`、`self_check_summary`、`self_check_finished_at`。升级任务只记录升级动作自身的预检查、执行步骤、日志、健康检查和最终状态。

- 外部 Tower、Prometheus 样本延迟、采集失败等业务数据问题由采集告警、任务中心和报表数据质量说明呈现。
- 升级动作健康检查失败仍按升级流程处理，不依赖平台自检结果。
```
