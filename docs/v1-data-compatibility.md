...
- v1 迁移包不应包含明文凭据。
- 如果 v1 数据中存在加密凭据，v2 优先使用当前 v2 凭据格式解密；解密失败时兼容 v1 Fernet 凭据格式，密钥种子优先使用 `SMARTX_CREDENTIAL_KEY`，其次使用 `SMARTX_SECRET_KEY`。
- 如果旧凭据仍无法解密，导入后需要用户重新录入或重新验证 Tower 连接。
- 任何导入、验证和日志都不得输出 Tower 明文密码或 API Token。
...
- 如需释放 SQLite 文件空间，可在服务管理的空间清理页执行“SQLite 清理并整理”；执行前会备份 `smartx.db`，随后清理运行态缓存并执行 VACUUM。
- SQLite 运行态缓存默认策略：`metric_snapshots` 最多保留 1 条，`collection_runs` 保留最近 7 天，`tasks` 保留最近 30 天且未确认告警/严重告警继续保留。
