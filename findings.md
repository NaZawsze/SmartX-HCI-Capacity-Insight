...
- 报告封面和页眉页脚不能使用 `SmartX 超融合平台` 等官方项目表述，当前项目名称统一为 `存储容量预测平台`。
- Word 字体需要使用开源字体，当前统一为 `Noto Serif` / `Noto Serif CJK SC`，避免依赖 Windows 专有字体。
- 客户版模板中的 `客户名称` 不应误写成 Tower 名称，封面保留空白 `客户名称` 字段；真实范围单独显示为 `Tower范围` 和 `集群范围`，多 Tower / 多集群时显示 `全部 Tower（N 个）`、`全部集群（N 个）`，报告正文需要明确具体 Tower 和集群。
- 统计窗口必须使用“用户选择窗口”和“实际采集窗口”的交集；刚部署时如果选择 365 天，但只有 14 天样本，Word 应显示实际 14 天样本窗口。
- 关键发现和运维建议不依赖 AI 服务，使用本地规则模板生成；重点 VM 名称、Tower/集群、容量、增长值、百分比和天数用 run 级加粗放大，便于客户阅读。VM 名称前后只保留 1 个空格做视觉分隔，数据值不额外加空格。
- 风险矩阵不使用 `单 VM 容量异常` 这类容易误导客户的表述，改为 `重点 VM 容量`，保持关注提示但不把单台 VM 直接定义为异常。
- `2.2` 容量增长趋势需要包含 Tower 列，避免多 Tower 场景只看到集群名。
- `2.3` 容量使用率可视化改为 DOCX 纯文本块进度条：使用 `█` 表示已用比例、`░` 表示剩余比例，展示当前使用率、90 天预测使用率、容量阈值和容量安全边际说明；多集群时默认展示当前使用率最高的集群，并写明 Tower/集群名称。

2026-06-08 第三轮口径修正结论：

- Word 客户版报告如果实际采集窗口只有 `2026-06-01 - 2026-06-08`，执行摘要和 KPI 不能写 `较上月增长`，否则会被理解为完整自然月环比。
- 客户版 Word 的增长标签改为按采集/导出窗口表达：`统计窗口增长`、`近 90 天样本增长`、`近 365 天样本增长`；底层计算继续使用已有样本，样本不足时从最早样本回退到当前样本。
- `2.1 范围基本信息` 不能只像单 Tower/单集群表，需显示 `Tower范围`、`Tower数量`、`集群范围`、`集群数量`，多 Tower/多集群时使用聚合范围摘要。
- 正文叙述中的 Tower 和集群名称需要和 VM 名称一样做重点强调，便于客户快速定位具体范围。

2026-06-08 报表导出 6 种时间区间 Profile 化结论：

- 不采用 6 份完整 Word/Excel 模板。完整模板会在后续样式、章节、表格修复时产生漂移，容易出现某个时间区间漏改。
- 不采用 1 个模板 + 6 套替换脚本。替换脚本对 DOCX XML 脆弱，容易误替换、漏替换，也难以保证 Word 和 Excel 口径一致。
- 采用 1 套客户版渲染系统 + 6 个时间区间 Profile。Profile 只负责窗口语义和文案策略，渲染器负责统一版式和数据填充。
- 6 个 Profile 固定为 `7/14/30/90/180/365`，与前端导出按钮和后端 `_normalize_period_days()` 支持范围一致。
- Profile 需要统一驱动 Word 和 Excel：窗口名称、增长指标标题、样本不足说明、章节重点和运维建议语气都从同一个 Profile 读取。
- 样本窗口仍按“用户选择窗口”和“实际采集窗口”的交集显示；选择 `365` 天但只有 17 天采集数据时，必须写实际统计窗口，并说明长期增长值按可用样本计算。

2026-06-09 Excel 客户版模板发现：

- 用户提供的 `存储容量预测分析报表_客户版.xlsx` 结构为 `封面`、`执行摘要`、`容量趋势`、`VM增长TOP20`、`日增长详情`，整体更接近客户交付版式。
- 模板优点：封面简洁、执行摘要使用 KPI 卡片、表头深蓝、数据区浅色底纹、VM Top 表左右并排、明细表冻结窗格，适合直接作为 v2 Excel 样式母版。
- 模板不能原样使用：包含 `SmartX 超融合平台`、硬编码客户/Tower/集群、固定样例数据和固定单位；实施时必须清理为 v2 项目名、真实范围、真实统计窗口和 bytes 格式化单位。
- Excel 后续不应继续从空 Workbook 手搓客户版样式；更稳妥方案是将模板纳入仓库，导出时加载模板并填充 v2 数据。
- 多 Tower/多集群时，封面保持摘要，正文/Sheet 使用范围明细表：每行一个 `Tower + 集群`，并在容量趋势、集群汇总和每集群 Sheet 中保留 Tower 列。

2026-06-09 Word 章节重排新需求：

- 用户要求 `执行摘要` 改成 `摘要`，仅指 Word 客户版正文第一章；Excel Sheet 暂不跟随改名。
- Word 第二页必须是目录：封面后立即插入目录，目录之后再进入正文。
- 当前客户版 Word 实际生效的是 `export.py` 后半段第二个 `build_report_docx()`，不是前半段旧轻量 Word 导出函数。
- `三  虚拟机增长分析` 和 `四  容量趋势图表` 需要按集群拆分，每个集群单独成目录项；VM 数据和图表必须只使用该集群范围内的数据。
- `五` 改为所有集群汇总报告，承载全局容量风险矩阵、全局运维建议和声明，不再作为单集群章节。
- Word 封面需要同时表达两种窗口：`统计窗口` 是平台实际用于分析的采集样本窗口，`本报表统计窗口` 是用户在导出时选择的 7/14/30/90/180/365 天窗口。选择窗口大于采集历史时，不能把缺失历史伪装成完整样本。
- Word 目录采用原生 TOC 字段，正文必须使用真实 `Heading 1/2/3` 样式；生成时写入 `w:updateFields=true`，让 Word/WPS 打开文档时更新目录。
- 原生 TOC 在未由 Word/WPS 刷新前只会显示占位文本；自动化测试应检查 `TOC \o "1-3" \h \z \u`、`updateFields` 和标题样式，而不是依赖目录页已展开后的页码。
