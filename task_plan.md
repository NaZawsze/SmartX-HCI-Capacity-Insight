...
- v2 提交策略：默认提交并推送到 `dev2`；本轮用户已明确要求同步 `dev`、`main` 并打 `v0.5.1u2` tag。
...
- 发布 `v0.5.1u2` 过渡平台版本。
- 写清升级包说明、更新内容、修复内容和过渡版本边界。
- 为后续 `upgrade-runner v0.3.1` 组件升级做准备，但本版本不升级 runner。
- 同步推送到 `dev`、`main` 并创建 `v0.5.1u2` tag。
...
- 不包含 Prometheus 历史数据。
- 无 SQLite schema 迁移时不包含 `migration`、`migration_steps` 或 `script.sandbox.v1`。
...
- 当前正式口径固定为 `v0.5.1u2`；源码 `VERSION`、README、文档、正式升级包和常规镜像 tag 必须保持一致。
